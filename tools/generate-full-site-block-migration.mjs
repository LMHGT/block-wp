...
try {
  ...
  for (const route of targetRoutes) {
    ...
    const bodyForBlocks = removeLeadingSkipLinkText(extraction.visibleText);
    const canonicalFullText = cleanText(["Skip to content", bodyForBlocks].filter(Boolean).join(" "));
...
async function loadRoute(page, stagingEntry, route) {
  const response = await page.goto(new URL(route, stagingBaseUrl).toString(), { waitUntil: "networkidle", timeout: 30000 });
  if (!response) throw new Error(`No browser response for ${route}.`);
  return { mode: "live-staging-browser", html: await response.text(), status: response.status() };
}
