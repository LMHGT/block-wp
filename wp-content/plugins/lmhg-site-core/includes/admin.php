...
function lmhg_site_core_render_seo_overview(): void {
	...
	foreach ( $query->posts as $post ) {
		...
		$stored_schema_type = trim( (string) get_post_meta( $post->ID, '_lmhg_schema_type', true ) );
		$schema_type = '' !== $stored_schema_type ? $stored_schema_type : ( function_exists( 'lmhg_site_core_default_schema_type_for_page' ) ? lmhg_site_core_default_schema_type_for_page( (int) $post->ID ) : 'WebPage' );
